...
def evaluate_degree_baseline(data) -> dict[str, float]:
    # Use follower_count as the simplest human-readable influencer score.
    follower_index = FEATURE_NAMES.index("follower_count")
    scores = data.x[:, follower_index]
    # Choose the threshold from the training split so the baseline stays fair.
...
def _threshold_from_train_scores(train_scores: torch.Tensor) -> float:
    # Median is a simple robust cutoff for this toy benchmark.
